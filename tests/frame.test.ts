...
import { baseWindow, clampFrame, frameToRegion, interpolateFrame, upsertKeyframe, removeKeyframeAt } from "@shared/frame";
...
describe("baseWindow", () => {
  ...
  test("узкий исходник: окно во всю ширину", () => {
    expect(baseWindow({ width: 500, height: 2000 }, output)).toEqual({ width: 500, height: 888 });
  });
});
...
describe("interpolateFrame", () => {
  ...
  test("до первого ключа держит первое значение", () => {
    expect(interpolateFrame(kfs, 0, { cx: 0, cy: 0, zoom: 1 })).toEqual({ cx: 100, cy: 200, zoom: 1 });
  });
  test("между ключами линейно", () => {
    expect(interpolateFrame(kfs, 2, { cx: 0, cy: 0, zoom: 1 })).toEqual({ cx: 200, cy: 300, zoom: 1.5 });
  });
  test("после последнего держит последнее", () => {
    expect(interpolateFrame(kfs, 10, { cx: 0, cy: 0, zoom: 1 })).toEqual({ cx: 300, cy: 400, zoom: 2 });
  });
  test("без ключей возвращает запасное значение", () => {
    expect(interpolateFrame([], 2, { cx: 5, cy: 6, zoom: 1 })).toEqual({ cx: 5, cy: 6, zoom: 1 });
  });
});
...
describe("keyframe list", () => {
  test("upsert заменяет ключ рядом по времени и сортирует", () => {
    const list = upsertKeyframe([{ t: 2, cx: 1, cy: 1, zoom: 1 }], { t: 1, cx: 0, cy: 0, zoom: 1 });
    expect(list.map((k) => k.t)).toEqual([1, 2]);
    const replaced = upsertKeyframe(list, { t: 2.005, cx: 9, cy: 9, zoom: 1 });
    expect(replaced).toHaveLength(2);
    expect(replaced[1]?.cx).toBe(9);
  });
  test("ключи на соседних кадрах при 60 fps остаются разными", () => {
    const list = upsertKeyframe([{ t: 1, cx: 1, cy: 1, zoom: 1 }], { t: 1 + 1 / 60, cx: 0, cy: 0, zoom: 1 });
    expect(list).toHaveLength(2);
  });
  test("remove удаляет ключ в момент времени", () => {
    expect(removeKeyframeAt([{ t: 1, cx: 0, cy: 0, zoom: 1 }], 1.005)).toHaveLength(0);
  });
});
