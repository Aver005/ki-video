...
describe("peaks", () => {
  test("полная амплитуда в минус не переполняет байт; C и TS совпадают", () => {
    ...
    expect(Array.from(computePeaks(samples, 2))).toEqual(Array.from(peaksTs(samples, 2)));
  });
});
...
describe("progress", () => {
  test("out_time_us в секунды, fps и speed", () => {
    expect(parseProgressBlock(["fps=59.5", "out_time_us=2500000", "speed=1.25x", "progress=continue"])).toEqual({ outTime: 2.5, fps: 59.5, speed: "1.25x" });
...
describe("project validation", () => {
  test("новый проект проходит, обрезанный — нет", () => {
    ...
    expect(isProjectLike({ ...createProject("p"), output: null })).toBe(false);
  });
});

describe("guard", () => {
  const req = (method: string, headers: Record<string, string>) => new Request("http://127.0.0.1:4777/api/x", { method, headers });
  test("GET со своего хоста проходит, чужой хост — нет", () => {
    expect(isOwnRequest(req("GET", { host: "127.0.0.1:4777" }))).toBe(true);
    expect(isOwnRequest(req("GET", { host: "evil.example:4777" }))).toBe(false);
  });
  test("POST проходит только со своей страницы", () => {
    expect(isOwnRequest(req("POST", { host: "localhost:4777", "sec-fetch-site": "same-origin" }))).toBe(true);
    expect(isOwnRequest(req("POST", { host: "localhost:4777", "sec-fetch-site": "cross-site" }))).toBe(false);
    expect(isOwnRequest(req("POST", { host: "localhost:4777", origin: "http://evil.example" }))).toBe(false);
...
describe("assets and probe", () => {
  ...
  test("probe: fps из дроби, звук может отсутствовать", () => {
    const r = toProbeResult({ streams: [{ codec_type: "video", codec_name: "hevc", width: 1728, height: 1080, avg_frame_rate: "60/1" }], format: { duration: "30.5" } });
    expect(r).toEqual({ duration: 30.5, width: 1728, height: 1080, fps: 60, videoCodec: "hevc", audioCodec: null });
  });
});
