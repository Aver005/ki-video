  ...
  return (
    <header className="header">
      ...
      <div className="header__meta">
        ...
        <span className={`dot ${status?.ok ? "dot--ok" : "dot--bad"}`} title={status?.ffmpeg ?? status?.error ?? "…"} />
        <span className="muted">{status?.hwaccel ? "NVENC" : status ? "CPU" : ""}</span>
      </div>
      <button className="btn btn--primary" onClick={() => setTab("export")} disabled={!project || project.clips.length === 0}>
        {running ? `Экспорт ${Math.round((exportJob?.percent ?? 0) * 100)}%` : "Экспорт"}
      </button>
    </header>
  );
}
