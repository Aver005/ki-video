...
function startTrim(e: React.PointerEvent, clipId: string, edge: "in" | "out", pxPerSec: number, initial: number): void {
  e.stopPropagation();
  e.preventDefault();
...
export function ClipBlock({ placement, pxPerSec }: ClipBlockProps) {
  ...
  const selected = useStore((s) => s.selection?.kind === "clip" && s.selection.id === clip.id);
  ...
  const thumbIndexes = Array.from({ length: count }, (_, i) => {
    ...
    return thumbs ? Math.min(thumbs.count, Math.max(1, Math.floor(t * thumbs.fps) + 1)) : 1;
  });
  ...
  return (
    ...
    >
      <div className="clip__thumbs">
        {asset?.status === "ready" &&
          thumbIndexes.map((n, i) => <img key={i} src={thumbUrl(asset.id, n)} alt="" style={{ width: width / count }} draggable={false} />)}
      </div>
      {asset?.peaks && <Waveform peaks={asset.peaks} from={clip.in} to={clip.out} width={Math.round(width)} height={WAVE_HEIGHT} />}
      <div className="clip__keys">
        {clip.frame.map((k) => (
          <span key={k.t} className="key" style={{ left: k.t * pxPerSec }} title={`ключ ${k.t.toFixed(2)}s`} />
        ))}
      </div>
      <div className="clip__handle clip__handle--l" onPointerDown={(e) => startTrim(e, clip.id, "in", pxPerSec, clip.in)} />
      <div className="clip__handle clip__handle--r" onPointerDown={(e) => startTrim(e, clip.id, "out", pxPerSec, clip.out)} />
    </div>
  );
}
