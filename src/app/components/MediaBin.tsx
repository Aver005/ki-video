...
export function MediaBin() {
  ...
  return (
    <div className="bin">
      ...
      >
        <input value={path} onChange={(e) => setPath(e.target.value)} placeholder="или путь к файлу…" aria-label="Путь к файлу" />
      </form>
      <div className="bin__list">
        {list.length === 0 && <div className="muted bin__empty">Добавь ShadowPlay-клипы, чтобы начать</div>}
        {list.map((asset) => (
          <AssetCard key={asset.id} asset={asset} progress={progress[asset.id] ?? 0} />
        ))}
      </div>
      {browser && (
        <FileBrowser
          onClose={() => setBrowser(false)}
          onPick={(paths) => void importPaths(paths).then((ok) => ok && setBrowser(false))}
        />
      )}
    </div>
  );
