...
export function ExportTab() {
  ...
  const sizeIndex = SIZES.findIndex((s) => s.width === output.width && s.height === output.height);
  return (
    <div className="tab-body">
      ...
      <div className="field-row">
        <label className="field">
          ...
          >
            <option value="h264">H.264 · {status?.encoders?.h264 ?? "…"}</option>
            <option value="hevc">H.265 · {status?.encoders?.hevc ?? "…"}</option>
          </select>
        </label>
        <label className="field">
          <span>FPS</span>
      ...
      <div className="row-actions">
        <button className="btn btn--primary" disabled={running || clips === 0 || !status?.ok} onClick={() => void startExport()}>
          {running ? "Идёт экспорт…" : "Экспортировать"}
        </button>
        {running && job && (
          <button className="btn btn--ghost" onClick={() => void api.cancelExport(job.id)}>
            Отменить
          </button>
        )}
      </div>
      {job && (
        <div className="export-status">
          <div className="progress">
            <div className={`progress__bar ${job.status === "error" ? "progress__bar--error" : ""}`} style={{ width: `${Math.round(job.percent * 100)}%` }} />
          </div>
          <div className="muted">
            {job.status === "running" && `${formatTime(job.outTime, false)} / ${formatTime(job.duration, false)} · ${job.speed}`}
            {job.status === "done" && `Готово за ${(((job.finishedAt ?? 0) - job.startedAt) / 1000).toFixed(1)} с`}
            {job.status === "error" && <span className="error">{job.error}</span>}
            {job.status === "cancelled" && "Отменено"}
          </div>
          {job.status === "done" && (
            <button className="btn" onClick={() => void api.reveal(job.outFile)} title={job.outFile}>
              Показать файл
            </button>
          )}
        </div>
