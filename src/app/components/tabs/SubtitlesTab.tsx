...
export function SubtitlesTab() {
  ...
  const addAtPlayhead = () => {
    ...
    setCues([...subtitles.cues, { id: crypto.randomUUID(), start: time, end: time + 2, text: "Реплика" }].sort((a, b) => a.start - b.start));
  };
  ...
  return (
    <div className="tab-body">
      ...
      <textarea
        ...
        rows={10}
        value={draft}
        placeholder={"старт конец текст\n0.5 2.0 Привет\n2 4 Смотри что будет\n\nили вставь SRT"}
        onChange={(e) => setDraft(e.target.value)}
        spellCheck={false}
      />
      <div className="row-actions">
        <button className="btn btn--primary" onClick={() => setCues(parseCues(draft))} disabled={draft === cuesKey}>
          Применить
        </button>
        <span className="muted">{subtitles.cues.length} реплик</span>
      </div>
      <p className="muted">Распознавание речи не встроено: вставь SRT из любого сервиса.</p>
    </div>
  );
}
