...
export class Player {
  ...
  private located(): { at: Located; asset: MediaAsset } | null {
    ...
    const asset = at ? this.source.assets[at.placement.clip.assetId] : undefined;
  ...
  private loop = (): void => {
    ...
    const isActive = this.activeAssetId === current.asset.id && !video.paused;
    const finished = video.ended || video.currentTime >= clip.out - END_EPSILON;

    if (isActive && finished) {
      this.time = clipEnd;
    } else if (!isActive || Math.abs(video.currentTime - current.at.sourceT) > DRIFT_LIMIT) {
      this.switchTo(current.asset, current.at.sourceT);
    } else {
      this.time = start + (video.currentTime - clip.in);
    }

    if (this.time >= clipEnd - END_EPSILON && clipEnd >= this.duration - END_EPSILON) {
  ...
  private switchTo(asset: MediaAsset, sourceT: number): void {
    ...
    video.muted = false;
    if (Math.abs(video.currentTime - sourceT) > SEEK_EPSILON) video.currentTime = sourceT;
  ...
  render(): void {
    ...
    if (current) {
      const video = this.video(current.asset);
      if (!this.playing && Math.abs(video.currentTime - current.at.sourceT) > SEEK_EPSILON) {
        video.currentTime = current.at.sourceT;
      }
      const frame = interpolateFrame(current.at.placement.clip.frame, current.at.localT, defaultFrame(current.asset));
      ...
      if (video.readyState >= 2) {
        ctx.drawImage(video, region.x * k, region.y * k, region.w * k, region.h * k, 0, 0, width, height);
      }
      ctx.filter = "none";
    }
