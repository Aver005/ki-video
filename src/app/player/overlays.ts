...
function drawText(ctx: CanvasRenderingContext2D, text: string, o: DrawTextOptions): void {
...
function animationState(layer: TextLayer, t: number): { alpha: number; grow: number } {
  const sinceStart = t - layer.start;
  const untilEnd = layer.end - t;
  if (layer.animation === "fade") {
    return { alpha: Math.min(1, sinceStart / 0.15, untilEnd / 0.15), grow: 1 };
  }
...
export function drawOverlays(ctx: CanvasRenderingContext2D, project: Project, t: number, scale: number): void {
