...
import { addKeyframe, deleteSelection, redo, seek, splitAtPlayhead, undo } from "@app/store/actions";
...
const withoutModifiers = (fn: () => void): Handler => (e) => {
...
export function useKeyboard(): void {
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      if (e.isComposing || isTyping(e.target) || document.querySelector(".modal")) return;
