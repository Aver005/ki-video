...
function apply(event: ServerEvent): void {
  switch (event.type) {
    case "asset":
      setState((s) => ({ assets: { ...s.assets, [event.asset.id]: event.asset } }));
      break;
    case "asset-progress":
      setState((s) => ({ assetProgress: { ...s.assetProgress, [event.id]: event.percent } }));
      break;
    case "asset-removed":
...
function resync(): void {
  void api
    .assets()
    .then((list: MediaAsset[]) => setState({ assets: Object.fromEntries(list.map((a) => [a.id, a])) }))
    .catch(() => undefined);
}
...
export function connectEvents(): () => void {
  ...
  const open = () => {
    ...
    socket.onmessage = (msg) => apply(JSON.parse(String(msg.data)) as ServerEvent);
    socket.onclose = () => {
