...
function cueLine(cue: SubtitleCue, width: number, height: number, y: number): string {
...
export function buildAss(project: Project): string {
  ...
  const cues = project.subtitles.cues.map((cue) => cueLine(cue, width, height, project.subtitles.y));
  const texts = project.texts.map((layer) => textLayerLine(layer, width, height));
