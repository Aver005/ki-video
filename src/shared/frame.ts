...
export function baseWindow(source: Size, output: Size): Size {
  ...
  const raw = byHeight.width <= source.width ? byHeight : { width: source.width, height: source.width / aspect };
...
export function interpolateFrame(keyframes: readonly FrameKeyframe[], t: number, fallback: FrameState): FrameState {
...
export function frameToRegion(frame: FrameState, source: Size, output: Size): Region {
...
export function clampFrame(frame: FrameState, source: Size, output: Size): FrameState {
  const base = baseWindow(source, output);
  const zoom = clamp(frame.zoom, 1, MAX_ZOOM);
  const w = base.width / zoom;
  const h = base.height / zoom;
...
export function upsertKeyframe(keyframes: readonly FrameKeyframe[], keyframe: FrameKeyframe): FrameKeyframe[] {
  const next = keyframes.filter((k) => Math.abs(k.t - keyframe.t) > KEYFRAME_EPSILON);
  next.push(keyframe);
  next.sort((a, b) => a.t - b.t);
  return next;
}

export function removeKeyframeAt(keyframes: readonly FrameKeyframe[], t: number): FrameKeyframe[] {
  return keyframes.filter((k) => Math.abs(k.t - t) > KEYFRAME_EPSILON);
}

export function findKeyframeAt(keyframes: readonly FrameKeyframe[], t: number): FrameKeyframe | undefined {
