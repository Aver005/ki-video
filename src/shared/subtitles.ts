...
const SRT_LINE = /\d{1,2}:\d{2}:\d{2}[,.]\d{1,3}\s*-->\s*\d{1,2}:\d{2}:\d{2}[,.]\d{1,3}/;

export function parseSrtTime(text: string): number | null {
  const m = SRT_TIME.exec(text);
  if (!m) return null;
  const [, h, mm, ss, ms] = m;
  return Number(h) * 3600 + Number(mm) * 60 + Number(ss) + Number((ms ?? "0").padEnd(3, "0")) / 1000;
}
...
function makeCue(start: number | null, end: number | null, text: string): SubtitleCue | null {
...
export function parseSrt(text: string): SubtitleCue[] {
  ...
  for (const block of text.replace(/\r/g, "").split(/\n\s*\n/)) {
    ...
    const cue = makeCue(parseSrtTime(from ?? ""), parseSrtTime(to ?? ""), body);
...
export function parseSimple(text: string): SubtitleCue[] {
  ...
  for (const raw of text.replace(/\r/g, "").split("\n")) {
    ...
    if (!m) continue;
    const cue = makeCue(parseSeconds(m[1] ?? ""), parseSeconds(m[2] ?? ""), (m[3] ?? "").replace(/\\n/g, "\n"));
    if (cue) cues.push(cue);
  }
  return sorted(cues);
}
...
export function printSimple(cues: readonly SubtitleCue[]): string {
  return cues.map((c) => `${c.start.toFixed(2)} ${c.end.toFixed(2)} ${c.text.replace(/\n/g, "\\n")}`).join("\n");
}
