...
function proxyEncoder(tools: FfmpegTools): string[] {
  ...
  if (encoder === "h264_nvenc") return ["-c:v", encoder, "-preset", "p1", "-rc", "vbr", "-cq", "30", "-b:v", "0"];
  return ["-c:v", "libx264", "-preset", "ultrafast", "-crf", "28"];
}

export function ingestArgs(tools: FfmpegTools, asset: MediaAsset, threads: number): string[] {
  const fps = thumbFps(asset.duration);
  const args = ["-hide_banner", "-y", "-nostats", "-loglevel", "error", "-progress", "pipe:1", "-threads", String(threads)];
  ...
  args.push("-map", "0:v:0", "-vf", `scale=-2:'2*trunc(min(${PROXY_HEIGHT},ih)/2)'`, ...proxyEncoder(tools), "-g", "30", "-pix_fmt", "yuv420p");
  if (asset.audioCodec) args.push("-map", "0:a:0", "-c:a", "aac", "-b:a", "96k");
  args.push("-movflags", "+faststart", PROXY_FILE);
  args.push("-map", "0:v:0", "-vf", `fps=${num(fps, 5)},scale=-2:${THUMB_HEIGHT}`, "-q:v", "5", join(THUMBS_DIR, "%04d.jpg"));
  if (asset.audioCodec) args.push("-map", "0:a:0", "-ac", "1", "-ar", String(PCM_RATE), "-f", "s16le", PCM_FILE);
  return args;
}
...
export async function ingest(
  ...
  await runFfmpeg(tools.ffmpeg, ingestArgs(tools, asset, threads), {
    cwd: cacheDir,
    signal,
    onProgress: (p) => onProgress(asset.duration > 0 ? Math.min(1, p.outTime / asset.duration) : 0),
  });
  const proxyInfo = await probe(tools.ffprobe, join(cacheDir, PROXY_FILE));
  const thumbFiles = (await readdir(join(cacheDir, THUMBS_DIR))).filter((f) => f.endsWith(".jpg"));
  return {
