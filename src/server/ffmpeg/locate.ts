...
export async function detectCaps(ffmpeg: string): Promise<EncoderCaps> {
  ...
  const nvencWorks = has("h264_nvenc") && hasCuda && (await nvencSmokeTest(ffmpeg));
  if (!nvencWorks) return SOFTWARE_CAPS;
  return { encoders: { h264: "h264_nvenc", hevc: has("hevc_nvenc") ? "hevc_nvenc" : "libx265" }, hwaccel: "cuda" };
}
...
async function nvencSmokeTest(ffmpeg: string): Promise<boolean> {
  const proc = Bun.spawn(
    [ffmpeg, "-hide_banner", "-loglevel", "error", "-f", "lavfi", "-i", "color=c=black:s=256x256:d=0.1", "-frames:v", "1", "-c:v", "h264_nvenc", "-f", "null", "-"],
...
export async function locateFfmpeg(configuredDir: string | null): Promise<FfmpegTools> {
  ...
  throw new Error("ffmpeg не найден: укажи ffmpegDir в ki.config.json или KI_FFMPEG_DIR");
}
