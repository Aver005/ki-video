...
export class ExportService
{
    ...
    ): Promise<ExportJob>
    {
        ...
        try
        {
            ...
            const outFile = join(
                this.exportsDir,
                `${safeName(project.name)}-${stamp()}.mp4`,
            )
            const assFile = hasOverlays(project) ? 'subs.ass' : null
            if (assFile)
                await Bun.write(join(workDir, assFile), buildAss(project))
    ...
    ): Promise<void>
    {
        ...
        try
        {
            await runFfmpeg(
                ffmpeg,
                ['-threads', String(this.threads), ...args],
                {
                    cwd,
                    ...
                    onProgress: (p) =>
                    {
                        const percent =
                            job.duration > 0
                                ? Math.min(0.999, p.outTime / job.duration)
                                : 0
                        this.emit(
                        {
