...
export async function openFileDialog(): Promise<string[]> {
  if (!isDialogSupported()) throw new Error("Диалог выбора файлов есть только на Windows; укажи путь вручную");
  const proc = Bun.spawn(["powershell", "-NoProfile", "-STA", "-NonInteractive", "-Command", SCRIPT], {
    stdout: "pipe",
    stderr: "pipe",
    windowsHide: true,
  });
  const [out, err, code] = await Promise.all([proc.stdout.text(), proc.stderr.text(), proc.exited]);
  if (code !== 0) throw new Error(err.trim() || `PowerShell завершился с кодом ${code}`);
  return out
    .split(/\r?\n/)
    .map((l) => l.trim())
    .filter((l) => l !== "");
