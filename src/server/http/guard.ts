...
export type Handler<P extends string = string> = (req: Bun.BunRequest<P>) => Response | Promise<Response>;
...
export async function readBody<T>(req: Request): Promise<T> {
  ...
  if (body === null || typeof body !== "object") throw new Error("Ожидался JSON-объект");
  return body;
}
