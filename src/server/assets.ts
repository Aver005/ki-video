...
{
    return Bun.hash(`${path}|${size}|${Math.floor(mtimeMs)}`)
        .toString(16)
        .padStart(16, '0')
}
...
export class AssetStore
{
    ...
    async load(): Promise<void>
    {
        ...
        {
            console.warn(
                '[assets] assets.json повреждён, начинаем с пустого списка',
            )
            return
        }
    ...
    private save(): void
    {
        ...
        this.saving = this.saving
            ...
            .catch((error: unknown) =>
                console.warn('[assets] не удалось сохранить индекс:', error),
            )
    }
    ...
    private async importOne(path: string): Promise<MediaAsset>
    {
        ...
        const pending = this.register(id, path).finally(() =>
    ...
    private async register(id: string, path: string): Promise<MediaAsset>
    {
        ...
        const done = this.queue.then(() =>
            this.process(asset, controller.signal),
        )
    ...
    async remove(id: string): Promise<boolean>
    {
        ...
        this.save()
        await rm(this.cacheDir(id), { recursive: true, force: true }).catch(
            (error: unknown) =>
