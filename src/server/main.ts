...
const assets = new AssetStore(tools, { dataDir: config.dataDir, threads: config.threads, nativePeaks: true });
await assets.load();
...
const isDev = process.env.NODE_ENV !== "production" && Bun.embeddedFiles.length === 0;
...
const server = Bun.serve({
  ...
  routes: {
    ...
    "/api/assets": {
      ...
      POST: guard(async (req) => {
        ...
        if (!Array.isArray(body.paths) || body.paths.some((p) => typeof p !== "string")) return fail("Нужен список путей");
        return json(await assets.import(body.paths));
      }),
    },
    "/api/assets/:id": {
      DELETE: guard(async (req) => json({ removed: await assets.remove(req.params.id) })),
    },
    "/api/assets/:id/proxy.mp4": guard((req) => {
      const path = cacheFile(req.params.id, PROXY_FILE);
      return path ? serveFile(req, path, "video/mp4") : fail("Файл не найден", 404);
    }),
    "/api/assets/:id/thumbs/:n": guard((req) => {
      const n = Number(req.params.n);
      if (!Number.isInteger(n) || n < 1 || n > 9999) return fail("Неверный номер", 400);
      const path = cacheFile(req.params.id, join(THUMBS_DIR, `${String(n).padStart(4, "0")}.jpg`));
      return path ? serveFile(req, path, "image/jpeg") : fail("Файл не найден", 404);
    }),
    ...
    "/api/export/:id": {
      ...
      DELETE: guard((req) => json({ cancelled: exporter.cancel(req.params.id) })),
    },
  ...
  fetch(req, srv) {
    ...
    if (url.pathname === "/ws") {
      return srv.upgrade(req) ? undefined : new Response("WebSocket only", { status: 426 });
    }
...
console.log(tools ? `ffmpeg: ${tools.version} · ${tools.caps.encoders.h264}/${tools.caps.encoders.hevc} · hwaccel ${tools.caps.hwaccel ?? "нет"}` : `ffmpeg: ${ffmpegError}`);
