...
async function findBrowser(): Promise<string | null> {
  ...
  return Bun.which("msedge") ?? Bun.which("chrome") ?? Bun.which("chromium") ?? Bun.which("google-chrome");
}

export async function openAppWindow(url: string, dataDir: string): Promise<string> {
...
export function revealInExplorer(path: string): void {
  ...
  Bun.spawn(["explorer", `/select,${path}`], { stdout: "ignore", stderr: "ignore" }).unref();
}
